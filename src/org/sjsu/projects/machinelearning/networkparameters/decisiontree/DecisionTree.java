...
public class DecisionTree {
	...
	/**
	 * When data is considered homogeneous and node becomes leaf and is labeled.
	 * If it is equal 1.0 then absolutely all data must be of the same label
	 * that node would be considered a leaf.
	 */
	private double homogenityPercentage = 0.90;

	/**
	 * Max depth parameter. Growth of the tree is stopped once this depth is
	 * reached. Limiting depth of the tree can help with overfitting, however if
	 * depth will be set too low tree will not be acurate.
	 */
	 ...
	 *            List of possible features.
	 * 
	 * @return Node after split. For a first invocation it returns tree root
	 *         node.
	 */
	protected Node growTree(List<DataSample> trainingData, List<Feature> features, int currentDepth) {

		Label currentNodeLabel = null;
		// if dataset already homogeneous enough (has label assigned) make this
		// node a leaf
		if ((currentNodeLabel = getLabel(trainingData)) != null) {
			// log.debug("New leaf is created because data is homogeneous: {}",
			// currentNodeLabel.getName());
		...
		if (stoppingCriteriaReached) {
			...
			// log.debug("New leaf is created because stopping criteria reached:
			// {}", majorityLabel.getName());
			return Node.newLeafNode(majorityLabel);
		}

		Feature bestSplit = findBestSplitFeature(trainingData, features); // get
																			// best
																			// set
																			// of
																			// literals
		...
		// splitData.stream().map(List::size).collect(Collectors.toList()));
		...
		for (List<DataSample> subsetTrainingData : splitData) { // add children
																// to current
																// node
																// according to
																// split
			if (subsetTrainingData.isEmpty()) {
				// if subset data is empty add a leaf with label calculated from
				// initial data
	...
	public Label classify(DataSample dataSample) {
		...
		while (!node.isLeaf()) { // go through tree until leaf is reached
			// only binary splits for now - has feature first child node(left
			// branch), does not have feature second child node(right branch).
	...
	/**
	 * Finds best feature to split on which is the one whose split results in
	...
	protected Feature findBestSplitFeature(List<DataSample> data, List<Feature> features) {
		...
		for (Feature feature : features) {
			...
			double calculatedSplitImpurity = splitData.parallelStream().filter(list -> !list.isEmpty())
	...
	/**
	 * Differs from getLabel() that it always return some label and does not
	 * look at homogenityPercentage parameter. It is used when tree growth is
	 * stopped and everything what is left must be classified so it returns
	 * majority label for the data.
	 */
	protected Label getMajorityLabel(List<DataSample> data) {
		// group by to map <Label, count> like in getLabels() but return Label
		// with most counts
		return data.parallelStream().collect(groupingBy(DataSample::getLabel, counting())).entrySet().stream()
